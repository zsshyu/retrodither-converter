...
export function adjustBrightnessContrast(
  ...
  for (let i = 0; i < data.length; i += 4) {
    for (let c = 0; c < 3; c++) {
      ...
      // Clamp
      resultData[i + c] = Math.max(0, Math.min(255, Math.round(value)));
    }
    resultData[i + 3] = 255;
  }
...
export function addGrayscaleNoise(imageData: ImageData, amount: number): ImageData {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    for (let c = 0; c < 3; c++) {
      resultData[i + c] = Math.max(0, Math.min(255, Math.round(data[i + c] + noise)));
    }
    resultData[i + 3] = 255;
  }
...
export function addRgbNoise(imageData: ImageData, amount: number): ImageData {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    resultData[i] = Math.max(0, Math.min(255, Math.round(
      data[i] + (Math.random() - 0.5) * 2 * noiseStrength
    )));
    resultData[i + 1] = Math.max(0, Math.min(255, Math.round(
      data[i + 1] + (Math.random() - 0.5) * 2 * noiseStrength
    )));
    resultData[i + 2] = Math.max(0, Math.min(255, Math.round(
